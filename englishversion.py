...
CMD3_EXEC = (
    ...
    'a unique value from the task-defined actions; [FORBIDDEN] printing only the option descriptions without the final decision;'
    ...
    '], then open the file with the system default program'
...
def llm_ask(messages, cfg=None):
    ...
    if os.environ.get("MAIN_PROGRAM_MOCK"):
        ...
        if text.startswith("Recognize the current actual") or "strictly output in JSON format" in text:
            r = MOCK_REPLY
        else:
            r = MOCK_COLLECT
        LLM_DIALOG_LOG.append({"send": messages, "receive": r, "ok": True, "mock": True})
...
def collect_info(cfg, emit=print, ask_user=None, mode="auto"):
    ...
    convert_prompt = (
        ...
        'Example format: {"<state_name>": "<current value>", ...} (use the EXACT state names from the criteria above; never invent or copy names). '
...
def run_training(task, cfg, emit=print, mock=False, open_ui=False):
    ...
    try:
        ...
        for run_round in range(1, max_fix + 1):
            ...
            done_markers = ("[DECISION]", "[DONE]", "[FINAL DECISION]", "[COMPLETED]", "[Finish]")
...
def build_gui():
    ...
    ttk.Label(root, text="What task would you like to complete? (the text below is only an EXAMPLE — replace it with YOUR own task)", font=("", 14, "bold")).grid(
